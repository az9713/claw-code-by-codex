...
from .commands import execute_command, find_commands, get_command, get_commands, render_command_index
...
from .tools import execute_tool, find_tools, get_tool, get_tools, render_tool_index
...
def build_parser() -> argparse.ArgumentParser:
    ...
    list_parser.add_argument('--limit', type=positive_int, default=32)

    commands_parser = subparsers.add_parser('commands', help='list mirrored command entries from the archived snapshot')
    commands_parser.add_argument('--limit', type=positive_int, default=20)
    ...
    tools_parser.add_argument('--limit', type=positive_int, default=20)
    ...
    route_parser.add_argument('--limit', type=positive_int, default=5)
    ...
    bootstrap_parser.add_argument('--limit', type=positive_int, default=5)
    ...
    loop_parser.add_argument('--limit', type=positive_int, default=5)
    loop_parser.add_argument('--max-turns', type=positive_int, default=3)
    ...
    load_session_parser.add_argument('--json', action='store_true')
    ...
    show_tool.add_argument('--json', action='store_true')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'commands':
        if args.json:
            modules = find_commands(args.query, limit=args.limit) if args.query else list(
                get_commands(
                    include_plugin_commands=not args.no_plugin_commands,
                    include_skill_commands=not args.no_skill_commands,
                )[: args.limit]
            )
            _json_print({'total': len(modules), 'limit': args.limit, 'query': args.query, 'entries': [asdict(module) for module in modules]})
        elif args.query:
    ...
    if args.command == 'tools':
        if args.json:
            modules = find_tools(args.query, limit=args.limit) if args.query else list(
                get_tools(
    ...
    if args.command == 'route':
        ...
        if args.json:
            _json_print([asdict(match) for match in matches])
            return 0
        if not matches:
