...
class PortRuntime:
    def route_prompt(self, prompt: str, limit: int = 5) -> list[RoutedMatch]:
        if limit <= 0:
            return []
        tokens = {token.lower() for token in re.findall(r'[A-Za-z0-9_]+', prompt)}
        by_kind = {
    ...
    def bootstrap_session(self, prompt: str, limit: int = 5) -> RuntimeSession:
        ...
        command_execs_list: list[str] = []
        tool_execs_list: list[str] = []
        for match in matches:
            if match.kind == 'command':
                command_entry = registry.command(match.name)
                if command_entry is not None:
                    command_execs_list.append(command_entry.execute(prompt))
                continue
            if match.kind == 'tool':
                tool_entry = registry.tool(match.name)
                if tool_entry is not None:
                    tool_execs_list.append(tool_entry.execute(prompt))
        command_execs = tuple(command_execs_list)
        tool_execs = tuple(tool_execs_list)
    ...
    def run_turn_loop(self, prompt: str, limit: int = 5, max_turns: int = 3, structured_output: bool = False) -> list[TurnResult]:
        ...
        engine.config = QueryEngineConfig(max_turns=safe_max_turns, structured_output=structured_output)
        matches = self.route_prompt(prompt, limit=limit)
        ...
        for turn in range(safe_max_turns):
