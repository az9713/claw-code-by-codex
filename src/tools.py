...
def _tool_index() -> dict[str, PortingModule]:
    # Keep first-seen entry for duplicate mirrored names.
    index: dict[str, PortingModule] = {}
    for module in PORTED_TOOLS:
        index.setdefault(module.name.lower(), module)
    return index
...
def get_tool(name: str) -> PortingModule | None:
    return _tool_index().get(name.lower())
...
def find_tools(query: str, limit: int = 20) -> list[PortingModule]:
    if not query:
        return list(PORTED_TOOLS[:limit])
    needle = query.lower()
    matches = [
        module
        for module in PORTED_TOOLS
        ...
        or needle in module.responsibility.lower()
    ]
