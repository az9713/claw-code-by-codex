...
class QueryEnginePort:
    ...
    def submit_message(
        ...
        summary_lines = [
            ...
            'Remaining budget tokens: pending',
        ]
        output = self._format_output(summary_lines)
        projected_usage = self.total_usage.add_turn(prompt, output)
        summary_lines[-1] = f'Remaining budget tokens: {self.remaining_budget_tokens(projected_usage)}'
        output = self._format_output(summary_lines)
        projected_usage = self.total_usage.add_turn(prompt, output)
    ...
    def remaining_budget_tokens(self, usage: UsageSummary | None = None) -> int:
        target_usage = usage or self.total_usage
        consumed = target_usage.input_tokens + target_usage.output_tokens
        return max(0, self.config.max_budget_tokens - consumed)
